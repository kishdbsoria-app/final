...
import {
  ...
  History,
  AlertCircle,
  HeartHandshake, // Added a more uplifting icon option
} from 'lucide-react';
...
// --- CONSTANTS ---
const ADMIN_PIN = '12345'; // Simple PIN for the prototype
const APP_NAME = 'KishDBSoria Dropping Area'; // Centralized name
...
export default function App() {
  ...
  // Auth State
  const [role, setRole] = useState(null); // 'admin' | 'seller' | 'buyer'
  ...
  // Login UI State
  const [loginMode, setLoginMode] = useState('menu'); // menu, admin, seller, buyer
  ...
  const [newItemLocation, setNewItemLocation] = useState('San Fernando');

  // Cash Out Selection State (For Admin)
  const [selectedSellerForCashout, setSelectedSellerForCashout] =
    useState(null);
  ...
  const handleSellerAuth = async () => {
    ...
    // Sanitize ID for use as a document ID
    const safeId = loginInputName
      .trim()
      .toLowerCase()
      .replace(/[^a-z0-9_]/g, '_');
    const userRef = doc(
  ...
  const handleAddItem = async (e) => {
    ...
    try {
      ...
      await addDoc(itemsRef, {
        ...
        sellerName: userName, // Logged in seller name
  ...
  // --- Admin Cash Out Logic (REVISED & FIXED) ---

  // 1. Group all 'claimed' items by seller
  const sellersWithBalance = useMemo(() => {
    if (role !== 'admin') return [];

    const groups = {};
    items.forEach((item) => {
      // Logic: Only show items that are 'claimed' but NOT YET 'cashed_out'
  ...
  const confirmCashOutForSeller = async () => {
    ...
    // Create a robust copy of IDs to avoid any state mutation issues
    ...
    try {
      // Process in small batches to ensure Firebase accepts them
      const batchSize = 250;
      ...
      // Give a tiny delay before closing to ensure UI updates feel smooth
      setTimeout(() => {
        alert(
          `Success! Payout recorded for ${sellerName}. Items moved to Archive.`
        );
        setSelectedSellerForCashout(null);
  ...
  const filteredItems = useMemo(() => {
    // BUYER PRIVACY RULE:
    // If user is a Buyer, and they haven't searched for anything yet, show NOTHING.
    if (role === 'buyer' && !searchTerm.trim()) {
      return [];
    }

    return items.filter((item) => {
      // 1. SELLER PRIVACY CHECK:
      // If user is a Seller, they can ONLY see their own items.
      if (role === 'seller' && item.sellerName !== userName) {
        return false;
      }

      // 2. Search Filter (Strict for Buyers)
      const searchLower = searchTerm.toLowerCase();

      let matchesSearch = false;

      if (role === 'buyer') {
        // Buyers can ONLY search by Buyer Name. They cannot search by item name or seller name.
        // This prevents them from browsing items by typing "Dress".
        matchesSearch = item.buyerName.toLowerCase().includes(searchLower);
      } else {
        // Admin and Seller can search everything
        matchesSearch =
      ...
      // Hide Archived/Cashed Out items from the main "All" list
      // They are only visible when the user explicitly clicks the "Archive/My History" tab
  ...
  const stats = useMemo(() => {
    // Determine which items to count based on role privacy
    let viewableItems = items;

    if (role === 'seller') {
      viewableItems = items.filter((i) => i.sellerName === userName);
    } else if (role === 'buyer') {
      // Buyers don't get to see global stats, only what they search for
      viewableItems = [];
    }
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full relative overflow-hidden">
          ...
          {loginMode === 'menu' && (
            <div className="space-y-3">
              ...
              >
                <div className="bg-purple-100 p-2 rounded-lg mr-4 group-hover:bg-purple-200">
              ...
              >
                <div className="bg-slate-100 p-2 rounded-lg mr-4 group-hover:bg-slate-200">
          ...
          {loginMode === 'admin' && (
            <div className="animate-in slide-in-from-right fade-in duration-300">
              ...
              <button
                ...
                className="w-full bg-purple-800 text-white py-3 rounded-lg font-semibold hover:bg-purple-900 mb-3"
              >
                Login
              </button>
              <button
          ...
          {/* SELLER FORM - Updated Colors to Pink */}
          {loginMode === 'seller' && (
            <div className="animate-in slide-in-from-right fade-in duration-300">
          ...
          {/* BUYER FORM - Updated Colors to Purple */}
          {loginMode === 'buyer' && (
            <div className="animate-in slide-in-from-right fade-in duration-300">
              <h2 className="text-lg font-bold text-slate-800 mb-4">
                Buyer Access
  ...
  return (
    ...
    <div className="min-h-screen bg-fuchsia-50">
        ...
        {role !== 'buyer' && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-white p-4 rounded-xl shadow-sm border border-pink-100">
              {role === 'seller' ? (
                <>
                  <div className="text-pink-500 text-xs font-semibold uppercase">
                    My Active Items
                  </div>
                  <div className="text-2xl font-bold text-slate-800">
                    {stats.dropped + stats.claimed}
                  </div>
                </>
              ) : (
                <>
            ...
            {/* Updated Stats Colors: Ready = Pink, Claimed = Purple */}
            <div className="bg-white p-4 rounded-xl shadow-sm border border-pink-100">
              <div className="text-pink-500 text-xs font-semibold uppercase">
                Ready to Pickup
              </div>
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-purple-100">
              <div className="text-purple-500 text-xs font-semibold uppercase">
                Claimed (Unpaid)
              </div>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-pink-100 mb-6 flex flex-col md:flex-row gap-4 justify-between items-center">
            ...
            <input
              ...
              placeholder={
                role === 'seller'
                  ? 'Search my items...'
                  : role === 'buyer'
                  ? 'Type YOUR NAME to find packages...'
                  : 'Search items, buyers, or sellers...'
              }
              value={searchTerm}
          ...
          <div className="flex gap-2 w-full md:w-auto overflow-x-auto pb-1 md:pb-0">
            ...
            {(role === 'admin' || role === 'seller') && (
              <button
                onClick={() => setStatusFilter('cashed_out')}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-pink-100 overflow-hidden">
          ...
          <div className="divide-y divide-pink-50">
            {filteredItems.map((item) => (
              ...
              <div
                key={item.id}
                className="group hover:bg-fuchsia-50 transition-colors"
              >
                {/* Desktop */}
                <div className="hidden md:grid grid-cols-7 gap-4 p-4 items-center">
                  ...
                  <div className="text-right flex items-center justify-end gap-2">
                    {/* STATUS BADGES & ACTIONS - Updated Colors */}
                    {item.status === 'dropped' ? (
                      <>
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-pink-100 text-pink-800">
                          Ready
                        </span>
                        {/* Only Admin can Mark Claimed now */}
                        {role === 'admin' && (
                          <button
                            onClick={() =>
                        ...
                        {/* Seller/Admin can cancel */}
                        {(role === 'admin' ||
                          (role === 'seller' &&
                            item.sellerName === userName)) && (
                          <button
                            onClick={() =>
                ...
                <div className="md:hidden p-4">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    {/* Mobile Status Badges - Updated Colors */}
                    {item.status === 'dropped' ? (
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-pink-100 text-pink-800">
                        Ready
                      </span>
                  ...
                  <div className="flex flex-col gap-1 mb-2">
                    <div className="text-xs text-slate-500 flex items-center gap-1">
                      <User className="w-3 h-3" /> Seller: {item.sellerName}
                    </div>
                  </div>
                ...
                {role === 'buyer' && !searchTerm ? (
                  <>
                    ...
                    <p className="text-sm text-pink-500">
                      Type your name to find your packages.
                    </p>
                  </>
                ) : (
                  <>
      ...
      {/* --- MODAL: DROP ITEM (Seller/Admin) - Updated Colors --- */}
      {isFormOpen && (
            ...
            <form onSubmit={handleAddItem} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  value={newItemBuyer}
                  onChange={(e) => setNewItemBuyer(e.target.value)}
                  className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-pink-500 outline-none"
                />
              </div>
              ...
              <button
                type="submit"
                className="w-full py-3 bg-pink-600 text-white rounded-lg font-semibold hover:bg-pink-700 shadow-md shadow-pink-200"
              >
                Confirm Drop
              </button>
            </form>
      ...
      {/* --- MODAL: ADMIN CASH OUT - Updated Colors --- */}
      {isCashOutModalOpen && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center overflow-y-auto">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl my-8">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  {selectedSellerForCashout
                    ? `Invoice for ${selectedSellerForCashout.name}`
                    : 'Select a seller to pay out'}
                </p>
              </div>
              <button
                onClick={() => {
            ...
            {/* Step 1: List of Sellers (If none selected) */}
            {!selectedSellerForCashout && (
              <div className="p-6">
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="text-left">
                            ...
                            <div className="text-xs text-slate-500">
                              {seller.items.length} items to claim
                            </div>
                          </div>
                        </div>
                        {/* Updated Text Color */}
            ...
            {/* Step 2: Invoice (If Seller Selected) */}
            {selectedSellerForCashout && (
              <div className="p-8">
                {/* Header - Updated Name */}
                <div className="text-center mb-8">
                ...
                <div className="bg-fuchsia-50 p-4 rounded-lg border border-pink-100 mb-6">
                  <div className="flex justify-between mb-2">
                    ...
                    <span className="font-bold text-lg text-slate-800">
                      {selectedSellerForCashout.name}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">Transaction ID:</span>
                    <span className="font-mono text-xs text-purple-600">
                      {selectedSellerForCashout.items[0]?.id
                        .substring(0, 8)
                        .toUpperCase()}
                      ...
                    </span>
                ...
                <table className="w-full mb-6 text-sm">
                  ...
                  <tfoot>
                    <tr className="border-t-2 border-slate-800">
                      <td
                        colSpan="2"
                        className="py-4 font-bold text-slate-800 text-right pr-4"
                      >
                        TOTAL PAYOUT:
                      </td>
                      <td className="py-4 font-bold text-xl text-purple-600 text-right">
                        ₱{selectedSellerForCashout.total}
                      </td>
                    </tr>
                  </tfoot>
                </table>
                ...
                {/* Actions - Updated Button Color */}
                <div className="flex gap-3 justify-end mt-6 border-t border-pink-100 pt-6">
                  <button
                    onClick={() => setSelectedSellerForCashout(null)}
                    className="px-4 py-2 text-slate-500 hover:text-pink-600"
                  >
                    Back
                  </button>
                  <button
